...
#else
    const char* pszModule = "AgentSmithCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ASC
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ASC
    // Mac: ~/Library/Application Support/ASC
    // Unix: ~/.ASC
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ASC";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "AgentSmithCoin.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good())
    {
        boost::filesystem::path ConfPath;
               ConfPath = GetDataDir() / "AgentSmithCoin.conf";
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "AgentSmithCoind.pid"));
