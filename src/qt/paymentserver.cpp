...
const int AGENTSMITHCOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString AGENTSMITHCOIN_IPC_PREFIX("AgentSmithCoin:");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(AGENTSMITHCOIN_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(AGENTSMITHCOIN_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(AGENTSMITHCOIN_IPC_CONNECT_TIMEOUT);
...
PaymentServer::PaymentServer(QApplication* parent) : QObject(parent), saveURIs(true)
{
    ...
    if (!uriServer->listen(name))
        qDebug() << tr("Cannot start AgentSmithCoin: click-to-pay handler");
    else
