...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // start the display unit, uses the default ("ASC")
    updateDisplayUnit();
}
...
{
    QString ret = QString("AgentSmithCoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non ASC unit input in lnReqAmount, we generate the URI with ASC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::ASC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
